...
void GridMazeGenerator::generate(
    Grid &grid, const Grid::Location &start, const Grid::Location &goal, std::vector<Grid::ChangeRecord> &record
)
{
    if (grid.height <= GridMazeGenerator::min_height || grid.width <= GridMazeGenerator::min_width)
    {
        throw std::invalid_argument(
            "Grid Maze Generator exception: Cannot generate maze. Provided grid is too small to generate a "
            "maze.\nMinimum grid size is "
            + std::to_string(GridMazeGenerator::min_width) + "x" + std::to_string(GridMazeGenerator::min_height)
    ...
    {
        throw std::invalid_argument(
            "Grid Maze Generator exception: Cannot generate maze. Provided start and/or goal are invalid.\nStart: "
