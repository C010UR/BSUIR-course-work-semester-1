...
Grid::Grid(const size_t width, const size_t height)
    ...
    for (size_t i = 0; i < height; i++) {
        this->grid.push_back(
            std::vector<Grid::CellType>(width, Grid::CellType::WALL));
    }
}

bool Grid::isInBounds(const Grid::Location &location) const {
    return 0 <= location.x && (size_t)location.x < width && 0 <= location.y &&
           (size_t)location.y < height;
}
...
std::vector<Grid::Location> Grid::neighbors(const Grid::Location &location,
                                            const unsigned distance,
                                            const bool is_passable) const {
...
Grid::cost_t Grid::heuristic(const Grid::Location &from,
